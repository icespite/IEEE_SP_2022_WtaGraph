...
def eval_model_inductive(args):
    ...
    for site_id in test_sites_map:
        print(
            "evaluating on testing site:",
            site_id,
            "  total testing edge in this site:",
            len(test_sites_map[site_id]),
        )
        ...
        new_edge_src_nids = []
        new_edge_dst_nids = []
        cur_nid = g.number_of_nodes()
        num_of_node_before_adding = g.number_of_nodes()
        ...
        for node in nodes_list_in_cur_g:  # find neighbors for each node in g
            if (
                ...
                for eid in train_g_dst_eids_map[node]:
                    ...
                    new_edge_dst_nids.append(
                        node_id_map[node]
                    )  # the dst nid will always be the id of current node
        ...
        if len(new_node_feats) > 0:  # we have neighbor found/added
            ...
            g.ndata["nf"][num_of_node_before_adding : g.number_of_nodes()] = th.stack(
                new_node_feats
            )

        if len(new_edge_src_nids) > 0:  # we have neighbor edge found/added
            g.add_edges(new_edge_src_nids, new_edge_dst_nids)
            g.edata["ef"][num_of_edge_before_adding : g.number_of_edges()] = th.stack(
                new_edge_feats
            )
            g.edata["e_label"][
                num_of_edge_before_adding : g.number_of_edges()
        ...
        test_mask[
            0:num_of_edge_before_adding
        ] = 1  # our target are the edges of testing site only
